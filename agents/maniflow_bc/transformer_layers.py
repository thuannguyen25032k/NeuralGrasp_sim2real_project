...
class AdaLN(nn.Module):
    """Adaptive LayerNorm — signal-modulated scale + shift."""
    ...
    def __init__(self, d_model):
        ...
        # Initialise to identity (no scale / shift) at the start of training
    ...
    def forward(self, x, t):
        ...
        t : (B, C)
        """
        scale, shift = self.modulation(t).chunk(2, dim=-1)  # (B, C) each
        return x * (1 + scale.unsqueeze(1)) + shift.unsqueeze(1)


class DummyLayer(nn.Module):
    """Shared utilities for pre-/post-norm and positional embedding addition."""
...
class AttentionLayer(DummyLayer):
    """Single attention layer (self- or cross-), with optional RoPE + AdaLN."""
    ...
    def __init__(self, d_model=256, dropout=0.1, n_heads=8, pre_norm=False,
        ...
        self.adaln = AdaLN(d_model) if use_adaln else None
        self.attention = MultiheadCustomAttention(d_model, n_heads, dropout=dropout)
...
class AttentionModule(nn.Module):
    """Stack of AttentionLayer + FFWLayer pairs."""
